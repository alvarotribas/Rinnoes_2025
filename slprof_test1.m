...
[f_half, Y_mn] = FFT(Tp, M, N, fc, fs, Vg, sig, alpha, c, R, fp, SNR_dB, range_bin, 30);
...
for i = 1:length(range_bins)
    n_idx = range_bins(i);
    gs_m = gs_prime(:, n_idx);
    plot(slow_time, abs(gs_m), 'Color', colors(i,:), 'DisplayName', ['n = ' num2str(n_idx)]);
end
xlabel('t [s]');
ylabel('y_{mn}(m, n)');
title('Examples of unfiltered range bins');
legend show;
grid on;
