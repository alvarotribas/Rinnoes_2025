...
% Single-point model
function [n, m, gs_prime] = ADC(Tp, M, fc, Vg, sig, alpha, c, R, fs, N)

    % Matrix with the dimensions pulses x samples
    gs_prime = zeros(M, N);
    ...
    for m_idx = 1:M
        ...
        % Center of fast-time window
        t_center = (N-1)/(2*fs);

        % target delay at pulse center
        x_center = sin(1*(t_start + t_center));
        tau_center = 2*(R + x_center)/c;

        for n_idx = 1:N
            % fast-time relative to pulse center
            t_rel = ((n_idx-1)/fs - t_center) - tau_center;
...
function [t_dac, g_dac, g_dac_matrix] = DAC(gs_prime, fs, up_factor)
    ...
    % Optional: reshape back to per-pulse matrix
    N_up = N * up_factor;
    if mod(length(g_up), N_up) == 0 && M*N_up == length(g_up)
        g_dac_matrix = reshape(g_up, N_up, M).';  % M x (N*up)
    else
        g_dac_matrix = [];
    end
...
[N, M] = deal(400, 400); % Number of pulses, Total number of samples in each pulse interval
fs = 10e8; % Sampling frequency (in Hz)
Ts = 1/fs; % Sampling period (in sec)
fc = 5e8; % Carrier frequency (in Hz)
...
% gs_prime
[~, ~, gs_prime] = ADC(Tp, M, fc, Vg, sig, alpha, c, R, fs, N);
